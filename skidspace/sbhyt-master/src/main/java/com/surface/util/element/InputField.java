...
public class InputField extends Gui {
    ...
    public TrueTypeFontDrawer font;
...
//    private final Animation cursorBlinkAnimation = new DecelerateAnimation(750, 1);
    ...
    public InputField(TrueTypeFontDrawer font) {
        this.font = font;
    }

    public InputField(TrueTypeFontDrawer font, float x, float y, float par5Width, float par6Height) {
        this.font = font;
        this.xPosition = x;
        this.yPosition = y;
    ...
    public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
        boolean flag = RenderUtils.isHovering(xPosition, yPosition, width, height, mouseX, mouseY);
    ...
    public void drawTextBox(int mouseX, int mouseY) {
        if (this.getVisible()) {
            ...
            if (this.isDrawingBackground()) {
                RenderUtils.drawRoundedRect(this.xPosition, this.yPosition, this.width, this.height, radius, realFillColor.getColor().getRGB());
            }
            ...
            if (backgroundText != null) {
                ...
                if (text.equals("") /* && !(textColor == 0)*/) {
                    font.drawString(backgroundText, xPos, yPos, -1);
                }
            }
            ...
            if (text.length() > 0) {
                ...
                j1 = this.font.getStringWidth2(password ? s1.replaceAll(".", "*") : s1) + .5f;
            }
            ...
            if (text.length() > 0 && cursorInBounds && cursorPos < text.length()) {
                ...
                j1 = font.getStringWidth2(password ? text.substring(cursorPos).replaceAll(".", "*") : text.substring(cursorPos));
            }
            ...
            cursorBlinkAnimation = LinearAnimation.animate(cursorBlinkAnimation, cursorBlinkAnimationStats ? 0 : 1, 0.1f);


      /*      RenderUtils.drawRect(k1 + 1, yPos - 2, .5f, font.getHeight() + 3,
                    new Color(1,1,1,cursorBlinkAnimation).getRGB());*/

 /*           if (selEnd != cursorPos) {
                int l1 = (int) (xPos + this.font.getStringWidth2(text.substring(0, selEnd)));
