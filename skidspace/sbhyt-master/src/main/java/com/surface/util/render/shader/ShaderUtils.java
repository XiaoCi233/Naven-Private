...
public class ShaderUtils {
        ...
        try {
            ...
            switch (fragmentShaderLoc) {
                ...
                case "gaussian":
                    ...
                    break;
                case "roundedRectTexture":
                    fragmentShaderID = createShader(new ByteArrayInputStream(roundedRectTexture.getBytes()), GL_FRAGMENT_SHADER);
                    break;
                case "bloom":
    ...
    public ShaderUtils(String fragmentShadersrc, boolean notUsed) {
        ...
        try {
            vertexShaderID = createShader(Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("surface/vertex.vsh")).getInputStream(), GL_VERTEX_SHADER);
        } catch (IOException e) {
    ...
    public ShaderUtils(String fragmentShaderLoc) {
        this(fragmentShaderLoc, "surface/vertex.vsh");
    }
    ...
    private String roundedRectTexture = "#version 120\n" +
            "\n" +
            "uniform vec2 location, rectSize;\n" +
            "uniform float alpha;\n" +
            ...
            "    vec2 uv = gl_TexCoord[0].st * rectSize;\n" +
            "    if ((uv.x < radius && uv.y < radius && corner.z == 0.0) ||\n" +
            "        (uv.x >= rectSize.x - radius && uv.y < radius && corner.y == 0.0) ||\n" +
            "        (uv.x < radius && uv.y >= rectSize.y - radius && corner.x == 0.0) ||\n" +
            "        (uv.x >= rectSize.x - radius && uv.y >= rectSize.y - radius && corner.w == 0.0)) {\n" +
            "        float smoothedAlpha = (1.0 - smoothstep(0.0, 1.0, roundSDF(rectHalf - uv, rectHalf - 0.1 - 1., 0.1))) * alpha;\n" +
            "        gl_FragColor = vec4(texture2D(textureIn, gl_TexCoord[0].st).rgb, smoothedAlpha);\n" +
            "    } else {\n" +
            "        float smoothedAlpha = (1.0 - smoothstep(0.0, 1.0, roundSDF(rectHalf - uv, rectHalf - radius - 1., radius))) * alpha;\n" +
            "        gl_FragColor = vec4(texture2D(textureIn, gl_TexCoord[0].st).rgb, smoothedAlpha);\n" +
            "    }\n" +
            "}\n";
    ...
    private String roundedRect = "#version 120\n" +
            ...
            "        (uv.x < radius && uv.y >= rectSize.y - radius && corner.x == 0.0) ||\n" +
            "        (uv.x >= rectSize.x - radius && uv.y >= rectSize.y - radius && corner.w == 0.0)) {\n" +
            "        float smoothedAlpha = (1.0 - smoothstep(0.0, 1.0, roundSDF(rectHalf - uv, rectHalf - 0.1 - 1., 0.1))) * color.a;\n" +
            "        gl_FragColor = vec4(color.rgb, smoothedAlpha);\n" +
            "    } else {\n" +
            "        float smoothedAlpha = (1.0 - smoothstep(0.0, 1.0, roundSDF(rectHalf - uv, rectHalf - radius - 1., radius))) * color.a;\n" +
            "        gl_FragColor = vec4(color.rgb, smoothedAlpha);\n" +
            "    }\n" +
            "}\n";
