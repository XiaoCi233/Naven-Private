...
public class AntiBots extends Module {
    ...
    private static long totalTime = 5L;
        ...
        if (e.getType() == EventType.RECEIVE) {
            if (e.getPacket() instanceof ClientboundPlayerInfoUpdatePacket) {
                ...
                if (uuids.containsKey(packet.getPlayerId())) {
                    ...
                    Notification notification = new Notification(NotificationLevel.INFO, "BOT MOMENT! Report VL*1  3VLs  Banned! (" + totalTime + "min)", 180000L);
                ...
                while (var9.hasNext()) {
                    ...
                    if (ids.contains(entityId)) {
                        ...
                        Notification notification = new Notification(NotificationLevel.INFO, "There are still 5min left before the report VL timeout! But you have to reset VLs.", 3000L);
                        BlinkFix.getInstance().getNotificationManager().addNotification(notification);
                        ids.remove(entityId);
                    }
                }
