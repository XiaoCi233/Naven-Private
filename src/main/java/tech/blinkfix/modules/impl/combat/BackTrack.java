...
import tech.blinkfix.utils.PacketSnapshot;
...
public class BackTrack extends Module {
    ...
    public void onPreTick(EventRunTicks event) {
        ...
        if (onlyKillaura.currentValue && !BlinkFix.getInstance().getModuleManager().getModule(Aura.class).isEnabled()) {
            clear();
            return;
        }
    ...
    public void onRender2d(EventRender2D event) {
        ...
        // Naven render style
        if (render.getCurrentValue() && (working || foundTarget)) {
            // Calculate progress based on delay - packets older than delay should be released
            float maxPackets = Math.max(1.0F, delay.getCurrentValue() / 50.0F); // Estimate max packets based on delay
            this.navenProgress.target = Mth.clamp((float) this.getPacketCount() / maxPackets * 100.0F, 0.0F, 100.0F);
            ...
            RenderUtils.drawRoundedRect(event.getGuiGraphics().pose(), (float)barX, (float)barY, barWidth, 5.0F, 2.0F, Integer.MIN_VALUE);
            RenderUtils.drawRoundedRect(event.getGuiGraphics().pose(), (float)barX, (float)barY, this.navenProgress.value, 5.0F, 2.0F, navenMainColor);
        }
    }
    ...
    public void onPacket(EventPacket e) {
        if (mc.player != null && mc.level != null) {
            if (e.getType() == EventType.RECEIVE) {
                if (working && !BlinkFix.getInstance().getModuleManager().getModule(LongJump.class).isEnabled() && !alink) {
                    Packet<?> packet = e.getPacket();
                    Velocity velocityModule = (Velocity) BlinkFix.getInstance().getModuleManager().getModule(Velocity.class);
        ...
        while (!this.packets.isEmpty()) {
            try {
                ...
                if (packet != null && mc.getConnection() != null) {
                    ...
                    BlinkFix.getInstance().getEventManager().call(eventBackrackPacket);
