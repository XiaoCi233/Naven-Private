...
import tech.blinkfix.events.impl.PostMotionEvent;
import tech.blinkfix.events.impl.WorldChangeEvent;
...
import xyz.gay.mixin.accessors.ServerboundMovePlayerPacketAccessor;
...
public class Disabler extends Module {
    ...
    private final BooleanValue grimDuplicateRotPlace = ValueBuilder.create(this, "GrimDuplicateRotPlace")
            ...
            .build()
            .getBooleanValue();
    private final BooleanValue transactionOrder = ValueBuilder.create(this, "ACAPlaceDisabler")
            .setDefaultBooleanValue(false)
            .build()
            .getBooleanValue();
...
//             if (packetSendEvent.isCancelled()) continue;
    ...
    public static float getPacketYRot(ServerboundMovePlayerPacket packet) {
        if (mc.gameMode == null) {
            ...
            try {
                return ((ServerboundMovePlayerPacketAccessor) packet).getYRot();
    ...
    public static float getPacketXRot(ServerboundMovePlayerPacket packet) {
        if (mc.gameMode == null) {
            ...
            try {
                return ((ServerboundMovePlayerPacketAccessor) packet).getXRot();
            } catch (Exception var3) {
    ...
    public static void setPacketYRot(ServerboundMovePlayerPacket packet, float yRot) {
        if (mc.gameMode != null) {
            try {
                ((ServerboundMovePlayerPacketAccessor) packet).setYRot(yRot);
    ...
    public static void setPacketXRot(ServerboundMovePlayerPacket packet, float xRot) {
        if (mc.gameMode != null) {
            try {
                ((ServerboundMovePlayerPacketAccessor) packet).setXRot(xRot);
            } catch (Exception var4) {
    ...
    private static boolean needDis;
    
    public static boolean getGrimTransactionOrder() {
        return mc.player != null && mc.level != null
                && BlinkFix.getInstance().getModuleManager().getModule(Disabler.class).isEnabled()
                && needDis && mc.player.tickCount > 30;
    }
    
    @EventTarget
    public void onTick(EventRunTicks e) {
        needDis = transactionOrder.getCurrentValue();
    }
    @EventTarget
