...
public class KillSay extends Module {
   ...
   FloatValue delay = ValueBuilder.create(this, "Delay")
         ...
         .build()
         .getFloatValue();
   ModeValue prefix = ValueBuilder.create(this, "Prefix").setDefaultModeIndex(0).setModes("None", "@", "!", "/shout ")
         .build().getModeValue();
   ...
   public void onPacket(EventPacket e) {
      if (e.getPacket() instanceof ClientboundPlayerInfoRemovePacket && e.getType() == EventType.RECEIVE
         ...
         for (UUID entry : packet.profileIds()) {
            ...
            if (playerInfo != null) {
               ...
               if (attackedPlayers.contains(playerName)) {
                  ...
                  List<String> styles = this.values.stream().filter(BooleanValue::getCurrentValue).map(Value::getName)
                        .toList();
                  if (!styles.isEmpty()) {
