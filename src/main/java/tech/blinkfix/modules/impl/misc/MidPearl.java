...
import render.impl.modules.tech.blinkfix.HUD;
import render.impl.modules.tech.blinkfix.NameProtect;
...
public class MidPearl extends Module {
    ...
    private void handleCounterLogic() {
        if (isAiming) {
            aimingTicks++;
            MidPearl.isThrowing = true;
            MidPearl.rotations = this.targetRotations;
...
@ModuleInfo(
        name = "MidPearl",
        description = "Throw ender pearl with middle click with HUD rendering",
        category = Category.MISC
)
public class MidPearl extends Module {
    ...
    private void updatePearlTracking() {
        Map<UUID, PredictedPearlInfo> updatedPearls = new HashMap<>();
        List<UUID> toRemove = new ArrayList<>();
    ...
    private class PredictedPearlInfo {
        ...
        public PredictedPearlInfo() {
            this.lastUpdateTime = System.currentTimeMillis();
        }

        public void update(ThrownEnderpearl pearl) {
            // Simplified pearl prediction - you may want to integrate the full physics logic from the second code
            this.pos = pearl.position();
            this.ticksToLand = (int) (pearl.position().distanceTo(mc.player.position()) / 1.5); // Rough estimation
        ...
        public void calculateAnimation() {
            if (this.pos == null) {
                this.finalWidth = 0;
                this.finalHeight = 0;
                this.currentWidth *= 0.85f;
                this.currentHeight *= 0.85f;
                return;
            }
