...
public class RenderUtils {
    ...
    public static void scissorStart(double x, double y, double width, double height) {
        ...
        double scissorX = x * scale;
        double scissorY = (window.getGuiScaledHeight() - (y + height)) * scale;
        double scissorW = width * scale;
        double scissorH = height * scale;
     ...
     * 绘制水平渐变矩形
     */
    public static void drawHorizontalGradientRect(PoseStack poseStack, float x, float y, float width, float height, int colorFrom, int colorTo) {
        RenderSystem.enableBlend();
        RenderSystem.defaultBlendFunc();
        RenderSystem.setShader(GameRenderer::getPositionColorShader);
        RenderSystem.setShaderColor(1.0f, 1.0f, 1.0f, 1.0f);
        ...
        buffer.begin(Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        buffer.vertex(matrix, x, y + height, 0.0F).color(redTo, greenTo, blueTo, alphaTo).endVertex();
        buffer.vertex(matrix, x + width, y + height, 0.0F).color(redTo, greenTo, blueTo, alphaTo).endVertex();
        buffer.vertex(matrix, x + width, y, 0.0F).color(redFrom, greenFrom, blueFrom, alphaFrom).endVertex();
        buffer.vertex(matrix, x, y, 0.0F).color(redFrom, greenFrom, blueFrom, alphaFrom).endVertex();
        tesselator.end();
