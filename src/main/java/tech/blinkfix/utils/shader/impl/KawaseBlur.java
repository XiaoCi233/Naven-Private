package tech.blinkfix.utils.shader.impl;

import tech.blinkfix.BlinkFix;
import tech.blinkfix.modules.impl.render.PostProcess;
...
import tech.blinkfix.utils.shader.ShaderHelper;
...
public class KawaseBlur {
    ...
    public void draw(int radius) {
        ...
        PostProcess postProcess = (PostProcess) BlinkFix.getInstance().getModuleManager().getModule(PostProcess.class);
        if (postProcess.getFastBlur()) {
            if (timer.delay(1000 / postProcess.getBlurFPS())) {
                timer.reset();
            } else {
                return;
            }
